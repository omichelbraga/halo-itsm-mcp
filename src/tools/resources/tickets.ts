...
export const ticketsConfig: ResourceConfig = {
  ...
  listParams: {
    ...
    include_custom_fields: z.string().max(500).optional().describe("Comma-separated list of custom field IDs to include"),
    ...
    requesttype: z.array(z.number().int()).max(100).optional().describe("Filter by request type IDs"),
    itil_requesttype: z.array(z.number().int()).max(100).optional().describe("Filter by ITIL request type IDs"),
    ...
    products: z.array(z.number().int()).max(100).optional().describe("Filter by product IDs"),
    flagged: z.array(z.number().int()).max(100).optional().describe("Filter by flagged ticket IDs"),
    excludethese: z.array(z.number().int()).max(100).optional().describe("Exclude these ticket IDs"),

    // Date search
    datesearch: z.string().max(100).optional().describe("Date field to search (e.g., 'dateoccured' for opened, 'datecleared' for closed)"),
    startdate: z.string().max(50).optional().describe("Start date for date range filter"),
    enddate: z.string().max(50).optional().describe("End date for date range filter"),

    // Text searches
    searchactions: z.boolean().optional().describe("Also search within ticket actions"),
    search_user_name: z.string().max(500).optional().describe("Search by user name"),
    search_summary: z.string().max(1000).optional().describe("Search by ticket summary"),
    search_details: z.string().max(2000).optional().describe("Search by ticket details"),
    search_reportedby: z.string().max(500).optional().describe("Search by reportedby field"),
    search_version: z.string().max(200).optional().describe("Search by software version"),
    search_release1: z.string().max(200).optional().describe("Search by release 1"),
    search_release2: z.string().max(200).optional().describe("Search by release 2"),
    search_release3: z.string().max(200).optional().describe("Search by release 3"),
    search_releasenote: z.string().max(1000).optional().describe("Search by release note"),
    search_inventory_number: z.string().max(200).optional().describe("Search by asset tag/inventory number"),
    search_oppcontactname: z.string().max(500).optional().describe("Search by opportunity contact name"),
    search_oppcompanyname: z.string().max(500).optional().describe("Search by opportunity company name"),
    ...
    })).max(50).optional().describe("Advanced search filters array. filter_type: 0=IN, 1=NOT IN, 2==, 3=<>, 4=LIKE, 5=NOT LIKE, 7=>, 8=>=, 9=<, 10=<="),
  },
  getParams: {
